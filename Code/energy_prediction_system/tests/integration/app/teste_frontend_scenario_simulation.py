...
from app.ui.main_window import DailySimulationWorker
from app.ui.main_window import HourlySimulationWorker
from app.ui.main_window import TemplateWorker
...
def mock_keyring():
    fake_keyring.clear()
    with (
        patch("app.manager.session_manager.keyring.set_password", side_effect=_set_password),
        patch("app.manager.session_manager.keyring.get_password", side_effect=_get_password),
        patch("app.manager.session_manager.keyring.delete_password", side_effect=_delete_password),
    ):
        yield
...
class TestModelsServiceContract:
    # --- get_all_models ---
    @patch("app.client.api_client.requests.get")
    def test_get_all_models_hits_correct_endpoint(self, mock_get):
        ...
        assert called_url.endswith("/models/"), (
            f"URL incorrecto: {called_url!r} — deve terminar em '/models/'"
        )
    ...
    def test_get_all_models_sends_no_body(self, mock_get):
        ...
        assert "json" not in call_kwargs or call_kwargs.get("json") is None, (
            "GET /models/ não deve enviar body JSON"
        )

    # --- activate_model ---

    @patch("app.client.api_client.requests.patch")
    def test_activate_model_hits_correct_endpoint(self, mock_patch):
        ...
        mock_patch.return_value = _make_response(
            200, {"model_name_id": 7, "is_active": True})

        ModelsService().activate_model(7)

        called_url: str = mock_patch.call_args[0][0]
        assert "/models/7/activate" in called_url, (
            f"URL incorrecto: {called_url!r} — deve conter '/models/7/activate'"
        )
    ...
    def test_activate_model_sends_correct_payload(self, mock_patch):
        ...
        mock_patch.return_value = _make_response(
            200, {"model_name_id": 3, "is_active": True})

        ModelsService().activate_model(3)

        sent_json = mock_patch.call_args[1].get("json", {})
        assert sent_json == {"is_active": True}, (
            f"Payload incorrecto: {sent_json!r} — deve ser {{'is_active': True}}"
        )
...
class TestSimulationServiceContract:
    ...
    @patch("app.client.api_client.requests.post")
    def test_get_template_hits_correct_endpoint(self, mock_post):
        ...
        mock_post.return_value = _make_response(200, {
            "frequency": "daily", "template_name": "average", "features": {}
        })

        SimulationService().get_template("daily", "average")

        called_url: str = mock_post.call_args[0][0]
        assert called_url.endswith("/simulations/templates"), (
            f"URL incorrecto: {called_url!r}"
        )
    ...
    def test_get_template_sends_correct_payload(self, mock_post):
        ...
        mock_post.return_value = _make_response(200, {
            "frequency": "hourly", "template_name": "storm", "features": {}
        })

        SimulationService().get_template("hourly", "storm")

        sent_json = mock_post.call_args[1].get("json", {})
        assert sent_json == {"frequency": "hourly", "template_name": "storm"}, (
            f"Payload incorrecto: {sent_json!r}"
        )
    ...
    def test_run_daily_hits_correct_endpoint(self, mock_post):
        ...
        mock_post.return_value = _make_response(
            200, {"predicted_mw": 500000.0})

        SimulationService().run_daily_simulation("average", 2025, 6, 3)

        called_url: str = mock_post.call_args[0][0]
        assert called_url.endswith("/simulations/daily"), (
            f"URL incorrecto: {called_url!r}"
        )
    ...
    def test_run_daily_payload_without_overrides(self, mock_post):
        ...
        assert "overrides" not in sent_json, (
            "Quando overrides=None, o campo não deve ser enviado no payload"
        )
        assert sent_json == {
    ...
    def test_run_daily_payload_empty_overrides_not_sent(self, mock_post):
        ...
        sent_json = mock_post.call_args[1].get("json", {})
        assert "overrides" not in sent_json, (
            "Quando overrides={}, o campo não deve ser enviado no payload"
        )
    ...
    def test_run_daily_payload_with_overrides(self, mock_post):
        ...
        mock_post.return_value = _make_response(
            200, {"predicted_mw": 700000.0})

        SimulationService().run_daily_simulation(
            "heatwave", 2025, 7, 4, overrides={"t2m": 40.0, "sp": 950.0}
        )
    ...
    def test_run_hourly_hits_correct_endpoint(self, mock_post):
        ...
        called_url: str = mock_post.call_args[0][0]
        assert called_url.endswith("/simulations/hourly"), (
            f"URL incorrecto: {called_url!r}"
        )
    ...
    def test_run_hourly_payload_full_structure(self, mock_post):
        ...
        SimulationService().run_hourly_simulation(
            "rainy", 2024, 11, 2, hour=8, overrides={"u10": 5.5}
        )
    ...
    @patch("app.client.api_client.requests.get")
    def test_get_available_templates_sends_frequency_as_query_param(
            self, mock_get):
        ...
        called_url: str = mock_get.call_args[0][0]
        assert called_url.endswith("/simulations/available-templates"), (
            f"URL incorrecto: {called_url!r}"
        )
        call_kwargs = mock_get.call_args[1] if mock_get.call_args[1] else {}
        assert call_kwargs.get("params", {}).get("frequency") == "daily", (
...
class TestDailySimulationWorkerContract:
    ...
    def test_worker_passes_all_args_to_service(self, qtbot):
        ...
        with patch("app.ui.main_window.SimulationService") as MockService:
            instance = MockService.return_value
            instance.run_daily_simulation.return_value = (
                {"predicted_mw": 500000.0}, 200
            )
    ...
    def test_worker_emits_service_data_unchanged(self, qtbot):
        ...
        with patch("app.ui.main_window.SimulationService") as MockService:
            MockService.return_value.run_daily_simulation.return_value = (
                expected, 200)
            ...
            assert blocker.args[0] is expected, (
                "Worker não deve modificar nem copiar os dados do Service"
            )
...
class TestHourlySimulationWorkerContract:
    ...
    def test_worker_passes_all_args_including_hour(self, qtbot):
        ...
        with patch("app.ui.main_window.SimulationService") as MockService:
            instance = MockService.return_value
            instance.run_hourly_simulation.return_value = (
                {"predicted_mw": 28500.0}, 200
            )
    ...
    def test_worker_passes_correct_hour_zero(self, qtbot):
        ...
        with patch("app.ui.main_window.SimulationService") as MockService:
            ...
            assert call_kwargs.get("hour") == 0, (
                "hour=0 (meia-noite) deve ser preservado — não descartado como falsy"
            )
    ...
    def test_worker_passes_frequency_and_template_to_service(self, qtbot):
        ...
        with patch("app.ui.main_window.SimulationService") as MockService:
            ...
            instance.get_template.return_value = (
                {"frequency": "hourly", "template_name": "rainy", "features": {}}, 200
            )
...
class TestLoadModelsWorkerContract:
    ...
    def test_worker_calls_get_all_models_with_no_args(self, qtbot):
        ...
        with patch("app.ui.views.model_management_view.ModelsService") as MockService:
            instance = MockService.return_value
    ...
    def test_worker_emits_empty_list_and_error_on_non_200(self, qtbot):
        ...
        with patch("app.ui.views.model_management_view.ModelsService") as MockService:
            MockService.return_value.get_all_models.return_value = (
...
class TestActivateModelWorkerContract:
    ...
    def test_worker_passes_model_id_to_service(self, qtbot):
        ...
        with patch("app.ui.views.model_management_view.ModelsService") as MockService:
            instance = MockService.return_value
            instance.activate_model.return_value = (
                {"model_name_id": 42, "is_active": True}, 200
            )
    ...
    def test_worker_emits_none_and_error_message_on_failure(self, qtbot):
        ...
        with patch("app.ui.views.model_management_view.ModelsService") as MockService:
            MockService.return_value.activate_model.return_value = (
                {"detail": "Model with id 99 not found"}, 404
            )
