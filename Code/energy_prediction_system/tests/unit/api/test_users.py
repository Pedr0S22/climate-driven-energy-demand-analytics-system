...
def test_login_lockout(client):
    ...
    for _ in range(settings.MAX_FAILED_ATTEMPTS):
        response = client.post("/api/auth/login", data={"username": "lock@example.com", "password": "wrongpassword"})
        assert response.status_code == 401
...
def test_logout(client):
    ...
    client.post("/api/auth/register", json=user_data)
    login_resp = client.post("/api/auth/login", data={"username": "logoutuser@example.com", "password": "password123"})

    # Correction: use the actual registered email
    login_resp = client.post("/api/auth/login", data={"username": "logout@example.com", "password": "password123"})
...
def test_get_me(client):
    ...
    client.post("/api/auth/register", json=user_data)
    login_resp = client.post("/api/auth/login", data={"username": "me@example.com", "password": "password123"})
    token = login_resp.json()["access_token"]
...
def test_rbac_client_access_admin_denied(client):
    ...
    client.post("/api/auth/register", json=user_data)
    login_resp = client.post("/api/auth/login", data={"username": "client@example.com", "password": "password123"})
    token = login_resp.json()["access_token"]
...
def test_rbac_admin_access_allowed(client, db):
    ...
    login_resp = client.post("/api/auth/login", data={"username": "admin@example.com", "password": "password123"})
    token = login_resp.json()["access_token"]
