...
def test_fill_nan_energy_15min_no_missing(cleaner):
    times = pd.to_datetime(["2023-01-01 10:00",
                            ...
                            "2023-01-01 10:45"],
                           utc=True)
    df = pd.DataFrame({"datetime": times, "Load_MW": [
...
def test_fill_nan_energy_15min_multiple_nans_uses_mean_last6(cleaner):
    ...
    df = pd.DataFrame({"datetime": times,
                       "Load_MW": [80.0,
                                   ...
                                   130.0]})
    df_filled = cleaner.fill_nan_energy(df.copy())
    valid_before = df.loc[df["datetime"] <
...
def test_hourly_aggregation_15min_to_hourly_mean(cleaner):
    times = pd.date_range(
        ...
        freq="15min",
        tz="UTC")
    df = pd.DataFrame({"valid_time": times, "latitude": [
        40.0] * 4, "longitude": [-8.0] * 4, "t2m": [10.0, 11.0, 12.0, 13.0]})
...
def test_create_daily_aggregation(cleaner):
    ...
    df_hourly = pd.DataFrame({"datetime": times, "Load_MW": [
                             100.0] * 48, "t2m": [10.0] * 48})
...
def test_full_pipeline_integration_mocked(mock_entsoe, mock_cds, cleaner):
    with tempfile.TemporaryDirectory() as base_dir:
        ...
        # Mock Energy
        times_e = pd.date_range(
            start=start_date,
            periods=4,
            freq="15min",
            tz="UTC")
        df_e = pd.DataFrame({"Unnamed: 0": times_e, "Load_MW": [
