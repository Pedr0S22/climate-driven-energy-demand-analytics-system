...
def test_fill_nan_energy_15min_no_missing():
    times = pd.to_datetime(
        ["2023-01-01 10:00", "2023-01-01 10:15", "2023-01-01 10:30", "2023-01-01 10:45"],
        utc=True,
    )
    df = pd.DataFrame(
        {
...
def test_fill_nan_energy_15min_multiple_nans_uses_mean_last6():
    times = pd.to_datetime([
        "2023-08-01 09:00", "2023-08-01 09:15", "2023-08-01 09:30", "2023-08-01 09:45",  # 09h
        "2023-08-01 10:00", "2023-08-01 10:15", "2023-08-01 10:30", "2023-08-01 10:45",   # 10h
        "2023-08-01 11:00", "2023-08-01 11:15", "2023-08-01 11:30", "2023-08-01 11:45",   # 11h — 2 NaN
    ], utc=True)

    df = pd.DataFrame({
        "Unnamed: 0": times,
        "Load_MW": [80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0, None, None, 125.0, 130.0]
    })
...
def test_fill_nan_energy_1h_plus_15min_no_missing():
    ...
    times_15 = pd.to_datetime(
        ["2022-01-01 02:00", "2022-01-01 02:15", "2022-01-01 02:30", "2022-01-01 02:45"],
        utc=True,
    )
    times = pd.concat([
        pd.Series(times_1h),
...
def test_fill_nan_energy_1h_plus_15min_multiple_nans_uses_mean_last6():
    ...
    times_15 = pd.to_datetime([
        "2022-01-01 02:00", "2022-01-01 02:15", "2022-01-01 02:30", "2022-01-01 02:45",  # 02h
        "2022-01-01 03:00", "2022-01-01 03:15", "2022-01-01 03:30", "2022-01-01 03:45",  # 03h — 2 NaN
        "2022-01-01 04:00", "2022-01-01 04:15"
    ], utc=True)
    ...
    df = pd.DataFrame(
        {
            ...
            "Load_MW": [
                100.0, 105.0,          # 1h
                110.0, 115.0, 120.0, 125.0,   # 02h
                130.0, 135.0, None, None,      # 03h — 2 NaN
                140.0, 135.0
            ],
        }
    )
...
def test_g115g1_1h_plus_15min_processed():
    ...
    df = pd.DataFrame(
        {"Unnamed: 0": times, "Load_MW": [100.0, 105.0, 110.0, 115.0, 120.0, 125.0]}
    )
...
def test_g15min_no_gaps_returns_same_df():
    ...
    df = pd.DataFrame({'valid_time': times, 't2m': [10, 11, 12, 13], 
                      'latitude': [40.0]*4, 'longitude': [-8.0]*4})
    df_filled = g15min(df)
...
def test_missingValuesFind_single_nan_imputes():
    ...
    times = pd.date_range("2023-01-01 10:00", periods=4, freq='15min', tz='UTC')
    df = pd.DataFrame({
        'valid_time': times, 't2m': [10, None, 12, 13],
        'latitude': [40.0]*4, 'longitude': [-8.0]*4
    })
...
def test_solar_imputation_night_zero():
    ...
    pd.testing.assert_series_equal(
        result.fillna(0), 
        pd.Series([0.0, 0.0, 0.0], index=times, dtype=float)
    )
...
def test_outliers_treatment_stl1_iqr_but_physical_ok_not_treated():
    ...
    df = pd.DataFrame({
        ...
        'latitude': [40.0]*9,
        'longitude': [-8.0]*9,
        'stl1': [20, 21, 22, 23, 50.0, 24, 25, 26, 27]  # ← 50 é outlier IQR mas OK físico
    })
...
def test_outliers_treatment_tp_iqr_but_physical_ok_not_treated():
    ...
    df = pd.DataFrame({
        ...
        'latitude': [40.0]*9,
        'longitude': [-8.0]*9,
        'tp': [1.0, 1.2, 1.5, 1.8, 20.0, 2.0, 2.2, 2.5, 3.0]  # ← 20 é IQR mas OK físico
    })
...
def test_hourly_aggregation_15min_to_hourly_single_column_mean():
    ...
    df = pd.DataFrame({
        ...
        'latitude': [40.0]*8,
        'longitude': [-8.0]*8,
        't2m': [10.0, 11.0, 12.0, 13.0, 15.0, 30.0, 25.0, 27.0]  # 10h=11.5, 11h=24.25
    })
    ...
    # Médias corretas
    assert pytest.approx(df_result.loc[0, 't2m'], rel=1e-6) == 11.5    # (10+11+12+13)/4
