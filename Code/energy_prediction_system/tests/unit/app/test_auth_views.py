...
def test_navigation_between_login_and_register(app_window, qtbot):
    ...
    qtbot.mouseClick(
        app_window.ui_login.register_link,
        Qt.MouseButton.LeftButton)
    assert app_window.stack.currentIndex() == 1  # Mudou para Registo

    qtbot.mouseClick(
        app_window.ui_register.login_link,
        Qt.MouseButton.LeftButton)
...
def test_login_validation_failure(mock_warning, app_window, qtbot):
    ...
    qtbot.mouseClick(
        app_window.ui_login.login_button,
        Qt.MouseButton.LeftButton)
...
def test_login_invalid_credentials(
    ...
    mock_warning.assert_called_once_with(
        app_window, "Login Failed", "Incorrect credentials.")
    assert app_window.stack.currentIndex() == 0
...
def test_register_success(mock_info, mock_api_response, app_window, qtbot):
    ...
    qtbot.mouseClick(
        app_window.ui_register.signup_button,
        Qt.MouseButton.LeftButton)
...
def test_logout_functionality(mock_clear_session, app_window, qtbot):
    ...
    qtbot.mouseClick(
        app_window.ui_user_homepage.logout_btn,
        Qt.MouseButton.LeftButton)
...
def test_login_server_error(mock_critical, mock_api_500, app_window, qtbot):
    ...
    mock_critical.assert_called_once_with(
        app_window, "Login Error", "Internal Server Error")
    assert app_window.stack.currentIndex() == 0
...
def test_register_server_error(mock_critical, mock_api_500, app_window, qtbot):
    ...
    qtbot.waitUntil(lambda: mock_critical.called)
    mock_critical.assert_called_once_with(
        app_window, "Registration Error", "Internal Server Error")
    assert app_window.stack.currentIndex() == 1
...
@patch("src.app.ui.main_window.QMessageBox.warning")
def test_login_invalid_request_format(
        mock_warning,
        mock_api_400,
        app_window,
        qtbot):
    ...
    qtbot.waitUntil(lambda: mock_warning.called)
    mock_warning.assert_called_once_with(
        app_window, "Login Failed", "Invalid Request format.")
    assert app_window.stack.currentIndex() == 0


@patch("src.app.ui.main_window.QMessageBox.warning")
def test_register_invalid_input_format(
        mock_warning,
        mock_api_response,
        app_window,
        qtbot):
    ...
    qtbot.waitUntil(lambda: mock_warning.called)
    mock_warning.assert_called_once_with(
        app_window, "Registration Failed", "Password too weak")
    assert app_window.stack.currentIndex() == 1


@patch("src.app.ui.main_window.QMessageBox.warning")
def test_login_invalid_request_format_list(
        mock_warning, mock_api_response, app_window, qtbot):
    mock_api_response(status_code=400, json_data={
    ...
    qtbot.waitUntil(lambda: mock_warning.called)
    mock_warning.assert_called_once_with(
        app_window, "Login Failed", "Field required")


@patch("src.app.ui.main_window.QMessageBox.warning")
def test_register_invalid_input_format_list(
        mock_warning, mock_api_response, app_window, qtbot):
    mock_api_response(status_code=400, json_data={
                      "detail": [{"msg": "Password too weak array"}]})
    app_window.stack.setCurrentIndex(1)
    ...
    mock_warning.assert_called_once_with(
        app_window,
        "Registration Failed",
        "Password too weak array")


@patch("src.app.ui.main_window.QMessageBox.critical")
def test_register_server_error_list(
        mock_critical,
        mock_api_response,
        app_window,
        qtbot):
    mock_api_response(status_code=500, json_data={
                      "detail": [{"msg": "Unknown array error"}]})
    app_window.stack.setCurrentIndex(1)
    ...
    qtbot.waitUntil(lambda: mock_critical.called)
    mock_critical.assert_called_once_with(
        app_window, "Registration Error", "Unknown array error")
