...
class PlotWidget(QtWidgets.QStackedWidget):
    def __init__(self, parent=None):
        ...
        # Initial empty plot
        self.ax.text(0.5, 0.5, "Waiting for data from API...", 
                    ha='center', va='center', fontsize=20, fontname='sans-serif')
    ...
    def update_chart(self, x_data, y_real, y_pred=None):
        ...
        if y_pred is not None:
            ...
            if len(full_pred_x) == len(full_pred_y):
                ...
                self.ax.plot(x_data[len(y_real):len(y_real)+len(y_pred)], y_pred, 
                             color='#000180', linewidth=2, linestyle='--', label='Forecast')
