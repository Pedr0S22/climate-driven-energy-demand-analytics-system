...
def get_hourly_prediction(
        historical_points: int = Query(
            3,
            ge=3,
            le=5,
            description="Number of historical hours to include"),
    predicted_points: int = Query(
            12,
            ge=1,
            le=24,
    ...
    try:
        ...
        logger.error(
            f"Error in hourly prediction endpoint: {e}",
            exc_info=True)
        raise HTTPException(
            status_code=500,
            detail="Internal Server Error during prediction") from e
...
def get_daily_prediction(
        historical_points: int = Query(
            ...
            ge=1,
            le=5,
            description="Number of historical days to include"),
    predicted_points: int = Query(
            7,
            ge=1,
            le=14,
    ...
    try:
        ...
        logger.error(
            f"Error in daily prediction endpoint: {e1}",
            exc_info=True)
        raise HTTPException(
            status_code=500,
            detail="Internal Server Error during prediction") from e1
