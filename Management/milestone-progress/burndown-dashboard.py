...
def generate_burndown(m_df, title, is_sprint, y_axis_label):
    ...
    today_dt = max(max_closed if pd.notna(max_closed) else kickoff,
    ...
    if not all_closed:
        plt.plot(bd_df['Date'], bd_df['Estimated'], linestyle='-.', color='red', linewidth=2.5, label='Estimated Delivery (Forecast)')
    ...
    # INTERACTIVE CHARTs
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=actuals['Date'], y=actuals['Remaining'], mode='lines+markers', name='Actual Burndown', line=dict(color='#1f77b4', width=3)))
    fig.add_trace(go.Scatter(x=bd_df['Date'], y=bd_df['Ideal'], mode='lines', name='Ideal Burndown', line=dict(color='gray', dash='dash', width=2)))
    
    if not all_closed:
        fig.add_trace(go.Scatter(x=bd_df['Date'], y=bd_df['Estimated'], mode='lines', name='Estimated Delivery', line=dict(color='red', dash='dashdot', width=2.5)))
